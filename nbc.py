...
class NaiveBayes:
    ...
    def compute_class_probabilities(self, y_train: np.ndarray) -> Dict[Any, float]:
        ...
        for i in range(len(classes)):
            class_probabilities[classes[i]] = (counts[i]+ self.alpha)/(len(y_train) + self.alpha*len(classes))
        # <<< END OF YOUR CODE <<<
    ...
    def compute_feature_probabilities(self, X_j_train: np.ndarray, y_train: np.ndarray) -> Dict[Tuple[Any, Any], float]:
        ...
        for c in class_labels:
            # >>> YOUR CODE HERE >>>
            X_j_class = X_j_train[y_train == c]

            total_samples_in_class = len(X_j_class)

            feature_counts = {feature_value: np.sum(X_j_class == feature_value) for feature_value in unique_features}

            for feature_value in unique_features:
                conditional_probability = (feature_counts[feature_value] + self.alpha) / (total_samples_in_class + self.alpha * len(unique_features))
                feature_probabilities[(feature_value, c)] = conditional_probability
            # <<< END OF YOUR CODE <<<
    ...
    def predict_probabilities(self, X_test: np.ndarray) -> np.ndarray:
            ...
            for c, label in enumerate(self.class_labels):
                ...
                for j in range(self.n_features):
                    ...
                    if (X_test[i, j], label) not in self.feature_probabilities[j]:
                        # >>> YOUR CODE HERE >>>
                        prob *= self.feature_probabilities[j][('NA', label)]
                        ...
                        # >>> YOUR CODE HERE >>>
                            #X_test[i, j] = feature_probabilities[j][(X_test[i, j], label)]
        ...
        # >>> YOUR CODE HERE >>>
            #normalized_probabilities = X[i, j]/np.sum(X[i, j] for j in range(self.n_features))
    ...
    def predict(self, probabilities: np.ndarray) -> np.ndarray:
        ...
        # Use the indices to get the actual class labels
        y_pred = self.class_labels[y_pred_indices]
        # <<< END OF YOUR CODE <<<
        return y_pred
    ...
    def evaluate(self, y_test: np.ndarray, probabilities: np.ndarray) -> Tuple[float, float]:
        ...
        # Note: Since it is a classification problem, the squared loss is calculated 
        # as the squared difference between the predicted probability for the true class 
        # and 1 (since the true label can be represented as a probability of 1).
        for i in range(len(y_test)):
            true_class_prob = probabilities[i, np.where(self.class_labels == y_test[i])[0][0]]
            squared_loss += (1 - true_class_prob) ** 2

        squared_loss /= len(y_test)
        # <<< END OF YOUR CODE <<<
